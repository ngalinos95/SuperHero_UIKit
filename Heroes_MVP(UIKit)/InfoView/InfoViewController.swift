...
class InfoViewController: UIViewController {
    ...
    var imageLoadingCompletion: (() -> Void)? // check the imageCompletion for unitesting
    // MARK: - Lifecycle
            ...
            if let imageUrl = URL(string: hero.image.url) {
                URLSession.shared.dataTask(with: imageUrl) { [weak self] (data, _, error) in
                    ...
                    DispatchQueue.main.async {
                        ...
                        // Call the completion block to indicate that the image is loaded
                        self.imageLoadingCompletion?()
                    }
                }.resume()
            }
            if segement == "Bio"{
